...
2. `aeko_get_domain_info(domain_id)` to ground the "vs us" comparison.
...
2. For each of the user's top 5-10 tracked prompts (derive from the domain's `brand_keywords[]` (from `aeko_get_domain_info`) + `aeko_search_research_prompts(scope=..., country=...)` if no tracked set yet):
