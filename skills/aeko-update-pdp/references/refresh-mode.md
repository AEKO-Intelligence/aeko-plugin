# Refresh mode — review JSON-LD only

Surgical JSON-LD update flow. Refresh mode patches **ONLY**
`AggregateRating.ratingValue`, `AggregateRating.reviewCount`, and `review[]`. Every byte of HTML outside
the existing JSON-LD blocks remains unchanged. It never turns into a full PDP rewrite.

Before any tool call, issue exactly one deferred-tool search for this mode:
...
```
/schedule every Monday 8am /aeko-update-pdp mode=refresh <product_id>
```
