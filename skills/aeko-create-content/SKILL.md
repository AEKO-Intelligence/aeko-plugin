...
**Changelog v0.13.1** — aeko.shop URL slug is now a **§6.3 hard gate**: `.meta.json` `slug` must be present and match `^[a-z0-9]+(?:-[a-z0-9]+)*$`, and — **only when the title is non-ASCII (Korean)** — must not equal the §5.5.3 romanized filename slug (the phonetic-gibberish reuse that 404'd). An already-English title whose romanized filename slug is itself meaningful may reuse it. Promotes §5.5.6 from advisory to required; recipe field tables flip to **required** with a matching acceptance gate.

**Changelog v0.13.0** — Pairs with the backend fix that **item-scopes the aeko.shop post key** (`source_content_id = aeko-item:{item_id}`), so re-generating + re-publishing a content item now **overwrites the same live post** instead of forking duplicates (`…-2 / …-3`). Skill corrections: removed the stale "`build_plan_md()` doesn't hydrate products" note (it does); a dropped product (missing store-synced `source_id`) and a failed image upload now **warn loudly** instead of silently producing a product-free / text-only post; clarified the hero slot is the standalone featured image (default-single-image-to-hero). `/aeko-publish-content`: corrected the false "re-publish the same variation overwrites" claim (same published variation is a no-op — regenerate a new variation to update), and added an optional, non-blocking claim-your-brand nudge. `/aeko-brand-kit` + this skill: clarified Brand Kit = **content voice kit** (the aeko.shop brand identity is derived on publish, keyed by domain; claiming is optional). Recipe: dropped pre-v1.4/inert framing; `source_content_id` marked advisory (backend derives the item-scoped key).
...
**REQUIRED for `aeko_shop` — enforced as a hard gate at §6.3.** The §5.5.3 slug is romanized for **local file organization** only. The **published aeko.shop URL** uses a *separate* slug carried in `<slug>.meta.json`'s top-level `slug` field. For a **non-ASCII (Korean) title**, do **NOT** reuse the romanized §5.5.3 filename slug for the URL — it transliterates to phonetic gibberish (`eseutetig-mogongpaegeul-…`) that reads as noise to humans and AI engines, and the §6.3 gate rejects a `slug` equal to the romanized filename slug of a non-ASCII title. (If the title is *already English*, the §5.5.3 filename slug is itself meaningful English — reusing it is fine and the gate allows it.) Write a **meaningful English** slug:
...
4. The aeko.shop backend uses this as the post's URL slug and enforces uniqueness (appends `-2`/`-3` on collision). Omitting `slug` (or, for a non-ASCII title, reusing the §5.5.3 romanized filename slug) makes the backend fall back to an ASCII transliteration of the Korean title — phonetic gibberish. This is a **hard gate** (§6.3): for `aeko_shop` you MUST supply a meaningful English `slug` — which for a Korean-titled post must differ from the romanized filename slug — or the draft fails and the item stays `pending`.
...
- `<slug>.meta.json` `slug` is present, non-empty, and matches `^[a-z0-9]+(?:-[a-z0-9]+)*$`. **When the §5.5.3 `resolved_title` contained any non-ASCII (e.g. Hangul) character, the `slug` MUST NOT equal this draft's romanized filename `<slug>` token** — that equality is the failure mode that produces phonetic-gibberish URLs (a romanized-from-Korean filename slug reused as the URL). When `resolved_title` is already pure ASCII/Latin, the filename slug is itself meaningful English, so a `slug` equal to it is fine (e.g. title `Summer Cooling Bedding — 2026 Guide` → `summer-cooling-bedding-2026-guide`). Hard gate. On failure, regenerate a meaningful English slug per §5.5.6 (one fix iteration per §6.5); if it is still missing, invalid, or a non-ASCII-title slug equal to the filename slug, leave the item `pending`.
