...
- **Language follows `target_language`.** KO brands: Vogue-Korea register — 감각적 sensory verbs, 해요체 or 합니다체 per content context, KO-market cultural cues (계절감, 라이프스타일 scenes). Global sellers: Vogue-US / Harper's Bazaar editorial English; don't fabricate cultural specificity — stay neutral if content context names no market. Other languages: that market's native magazine register.
...
- Single artifact, markdown only: `./aeko-artifacts/<domain_id>/<item_id>/magazine/<slug>__magazine.md`
  (path and `<slug>` per SKILL.md §A).
