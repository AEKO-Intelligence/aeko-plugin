...
- **Voice on the owned account** (`blog.naver.com/<brand>`). Default 해요체 unless content context says otherwise. Never fake a 1인칭 personal-reviewer voice — C-Rank penalizes it (opt-in only when prepping for a third-party influencer's blog). 저희 fine; 저 only with a named editor byline. Drop "한국 브랜드" framing (`[[feedback_aeko_brand_mark_and_scope]]`).
...
- Body 1,500–3,000자; out-of-band ≥50% → hard fail one fix iteration, else soft warn.
- Concrete-scene hook present (specific time/place/product/measurement); generic thesis or greeting openers fail.
- ≥2 H2 sections. ≥6 image slots with alt text (real `![alt](url)` when `media_by_channel[naver_blog]` is set, else `media_specs:` entries from this recipe's media guidance). Tags 5–10.
...
- Single artifact, markdown only: `./aeko-artifacts/<domain_id>/<item_id>/naver_blog/<slug>__naver_blog.md`
