...
- **Voice on the owned subdomain** (`<brand>.tistory.com`). Default 합니다체 (expertise register), 해요체 if content context specifies warmer. Tistory sits between magazine and Naver Blog — anecdote serves an explanatory point, never the whole arc. Do not fake 1인칭 or third-party-reviewer voice. No "한국 브랜드" framing.
...
- **Body 1,500–3,000자**, Hangul char count. Paragraphs ≤300자. ≥1 list or table block. Images 3–8장 at section boundaries, each with `*alt: ...*` below.
...
- ≥3 `## H2` sections, each answer-first with a declarative "X is Y" opener; question/anecdote/TOC openers count as misses. <3 → hard fail one fix iteration.
- Body 1,500–3,000자; out-of-band ≥50% → hard fail, else soft warn. ≥1 list/table block (not in 태그 section).
- ≥3 image slots with alt (real images when `media_by_channel[tistory]` is set, else `media_specs:` entries from this recipe's media guidance). Tags 5–10.
- No hard-CTA substrings (`지금 구매`, `구매하러`, `Buy now`, `Click here`, `클릭하세요`). No `[Image]`/`[Photo]`/`[placeholder]` markers; the coordinator re-checks placeholders in SKILL.md Step 6.

## File output

- Single artifact, markdown only: `./aeko-artifacts/<domain_id>/<item_id>/tistory/<slug>__tistory.md`
