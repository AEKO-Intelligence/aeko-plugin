...
3. Derive brand fields (`name`, `url`, `logo`) from `aeko_get_domain_info(domain_id)` and the Plan prose. If a field isn't available, omit it from the Organization schema rather than guess.
...
- Include any optional keys it surfaces (e.g., `Organization.foundingDate`, `WebSite.potentialAction.SearchAction`) when corresponding data exists in the domain info.
