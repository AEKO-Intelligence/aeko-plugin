...
3. Pull entity/site fields from `aeko_get_domain_info(domain_id)` and Plan/context (`brand_name` or domain name, tagline/description if surfaced, logo URL, sameAs links, key pages).
...
- Include any optional keys it surfaces (e.g., `Organization.foundingDate`, `WebSite.potentialAction.SearchAction`) when corresponding data exists in domain info or Plan/context.
