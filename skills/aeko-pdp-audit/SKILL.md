...
Answer one question: **Is this product page ready to be cited?** This is a read-only, zero-account audit.
Do not check for an AEKO account, call an AEKO tool, require a connector, or modify anything. URL audits use
Bash to run one bundled read-only fetcher; that is a weaker enforcement boundary than the former shell-free
design, so obey the narrow command boundary in Step 1 literally.
...
- URL: resolve `scripts/fetch_evidence.py` relative to this `SKILL.md`, then use Bash only to run
  `python3 <skill-directory>/scripts/fetch_evidence.py --mode page <url>`, passing the user-supplied URL as
  one safely quoted positional argument. Parse its one `aeko_fetch_evidence/v2` JSON object from stdout and
  require `mode: page`. This JSON—not WebFetch's
...
The bundled fetcher in page mode accepts one HTTP(S) URL, derives only same-origin `/robots.txt`, rejects credentials and
cross-host redirects, fetches no discovered asset, stores no cookie, and applies byte, redirect, per-request,
and total-time caps. If it emits `fatal_error`, a truncated body, or a fetch error, report that limitation and
...
Choose the product-detail root using semantic elements and platform selectors such as `main`, `article`,
`#prdDetail`, `#productDetail`, `.product-detail`, or their evident equivalents. Exclude global navigation,
global header/footer, cookie banners, unrelated review bodies, and Q&A threads. Retain product-detail tabs
and in-root navigation as `chrome`, and retain in-root merchandising widgets and platform policy text under
their own tags so the bot-view box remains complete. If no detail root is identifiable, use the page body
and disclose that fallback.

Walk the chosen root in DOM order and assign stable positions:

- each non-empty direct content block: `block_001`, `block_002`, ...
...
For a URL, use `detail_root.selector`, `detail_root.text_segments`, and `detail_root.image_components` from
the fetcher as the positional-map inputs. An image component includes
...
`detail image 3 — size chart` or `block 2 — care instructions` alongside the stable ID.
...
- `platform_boilerplate` — commerce-policy/support text appended by the storefront platform or theme; or
- `chrome` — in-root tabs, navigation labels, pagination, controls, review-summary interface text, and
  similar interface text.
...
facts links it to that item. Never count a price/name list, generic interface text, policy text, review
summary, or an ambiguous block merely because it sits inside the detail root. Everything that does not
describe this product must retain one of the named non-product tags above.
...
The first recognized commerce heading starts the boilerplate boundary. After excluding unrelated review
bodies, Q&A threads, and global footer content as already required, classify everything from that heading
to the end of the included detail root as `platform_boilerplate` unless a later block is clearly interleaved
product content. A carve-out needs positive evidence: the block names this product or contains unique
product facts and has its own product-content heading/module. Generic shipping, payment, return, refund, and
support wording remains boilerplate even when the merchant customized it.
...
Create one deterministic `machine_readable_text` string from text nodes in the product-detail root, and
four deterministic segment strings from the positional tags: `product_copy_text`,
`platform_boilerplate_text`, `merchandising_widget_text`, and `chrome_text`.
...
exactly once apart from normalized inter-segment separators. Never hide boilerplate to make the page look
cleaner. For an image-only page, the product-copy fence should remain empty or nearly empty.
...
page's <total_character_count> machine-readable characters are platform-appended payment, shipping,
returns, or support text; <merchandising_widget_character_count> characters promote other products; only
<product_copy_character_count> characters describe this product.` Name the platform only when the HTML
provides evidence. Boilerplate or merchandising dominance alone is `medium`; when essential product facts
...
Fetch or open each numbered detail image as an image and inspect the pixels directly with native vision. Do
not use, install, call, or simulate an OCR library. Do not infer a claim that is not visibly present. Preserve
units, qualifiers, ranges, exceptions, and source language exactly.
...
but extract zero product facts from a store-wide promo and label it plainly in the human list. Payment,
loyalty, membership, installment, and coupon terms from such a banner are not product facts.

Extract every disclosed fact, with special attention to these stable categories:
...
```json
{
  ...
  "facts": [
    {
      "source_id": "detail_image_003",
      "source_label": "detail image 3",
      "category": "size_chart",
      "fact": "M: 총장 68 cm, 가슴단면 54 cm",
      "evidence_verbatim": "M 총장 68 가슴단면 54"
    }
  ],
  "unreadable_images": [
    {
      "source_id": "detail_image_007",
      "asset_url": "https://example.com/detail-7.jpg",
      "reason": "HTTP 403"
    }
  ]
}
...
the number of objects in `facts`. `analyzed_image_count` counts successfully inspected image components.
Keep keys, schema value, source IDs, and category values exactly as shown. `source_label` mirrors the user's
language, while `source_id` never changes. This block is the handoff input for `/aeko-pdp-build`.
...
## Handoff
Image facts prepared for: /aeko-pdp-build
...
merchandising-widget, platform-boilerplate, chrome, image, fact, and schema-field counts may appear as factual numeric metrics;
...
- Never writes a file, edits a PDP, changes crawler policy, or queues an action. This skill now has Bash
  pre-approval, which is a genuinely weaker guardrail than having no shell: use it only for the bundled
  `fetch_evidence.py --mode page`. The script performs GET requests and stdout JSON only; it has no write, cookie, credential,
  or discovered-URL fetch path.
